...
def define_discriminator(image_shape):
    ...
    # source image input
    # Image we want to convert to another image
    in_src_image = Input(shape=image_shape)
    # target image input
    # Image we want to generate after training.
...
def define_generator(image_shape=(256, 256, 3)):
    ...
    g = Conv2DTranspose(image_shape[2], (4, 4), strides=(2, 2), padding="same", kernel_initializer=init)(
        d7)  # Modified
    # Generates images in the range -1 to 1. So change inputs also to -1 to 1
...
def define_gan(g_model, d_model, image_shape):
    ...
    for layer in d_model.layers:
        if not isinstance(layer, BatchNormalization):
            # Descriminator layers set to untrainable in the combined GAN but
...
def summarize_performance(step, g_model, dataset, n_samples=3):
    ...
    filename2 = "spiral_model_%06d.h5" % (step + 1)
